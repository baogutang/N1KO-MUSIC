...
前往 **[GitHub Releases](https://github.com/baogutang/N1KO-MUSIC/releases/latest)** 下载最新版本：
...
> ⚠️ **macOS 首次打开提示「无法验证开发者」**：前往「系统设置 → 隐私与安全性」，点击「仍要打开」即可。或在终端执行：
> ```bash
...
沉浸式全屏播放体验，专辑封面动态模糊渐变背景，实时同步滚动歌词，支持点击歌词跳转。支持方形封面与圆形旋转黑胶两种风格。
...
### 🎵 音乐体验
- **全屏播放器** — 专辑封面动态模糊渐变背景，平滑颜色过渡，沉浸式视觉体验
- **双封面风格** — 方形封面 / 圆形旋转黑胶唱片，在设置中自由切换
- **智能歌词** — 实时滚动高亮，点击歌词精准跳转，支持自定义远程歌词接口
- **Hi-Fi 无损音质** — 支持无损（FLAC / WAV / ALAC）、高音质（320kbps）、省流多档可选
- **播放队列** — 随机 / 列表循环 / 单曲循环，侧滑抽屉查看队列，拖拽排序，下一首插队

### 📚 音乐库与发现
- **音乐库** — 歌曲、专辑、歌手、歌单一体化浏览
- **歌曲详情页** — 完整标签信息（码率、格式、时长、年份、流派等）
- **专辑 / 歌手详情** — 完整曲目列表，一键播放整张专辑或全部作品
- **歌单管理** — 查看和播放服务器上的歌单
- **为你推荐** — 基于你的音乐库随机发现好音乐
- **全局搜索** — 跨所有内容的全文搜索
- **最近播放** — 本地播放历史，精美时间轴展示
...
### 🖥️ 桌面应用
- **macOS** — 原生窗口样式，Apple Silicon (arm64) + Intel (x64) 全平台支持
- **深色 / 浅色主题** — 跟随系统或手动切换，多种主题色可选
- **基于 Tauri 2 构建** — 轻量快速，原生性能，安装包体积小
